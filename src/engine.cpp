...
bool is_cast_by_party_member(const CombatEvent& event)
{
    ...
    return event.source_raid_flag == "0x511" ||
           event.source_raid_flag == "0x1511";
}
...
void ShotCallEngine::handle_player_event(const CombatEvent& event)
{
    ...
    if (is_battle_rez(event.spell_id) &&
        event.event_type == "SPELL_CAST_SUCCESS") {
        ...
        auto& player_crowd_control_map =
            roster_crowd_control_[player_iter->first];
    ...
    for (size_t spell_idx = 0; spell_idx < enemy.spells.size(); ++spell_idx) {
        ...
        for (size_t i = 0; i < iterations; ++i) {
            ...
            std::tuple<std::string, std::string, ch::time_point<ch::system_clock>> shotcall{
                std::make_tuple(enemy.id, spell.callout,
...
void ShotCallEngine::process_shotcalls()
{
    while (!shot_call_queue_.empty()) {
        ...
        auto time_until_call =
            ch::duration_cast<ch::milliseconds>(call_time - time_now);
...
void ShotCallEngine::identify_player(const CombatEvent& event)
{
    ...
    Player new_player(event.source_id, event.name, p_class, interrupt,
                      crowd_control_m);
...
bool is_battle_rez(int spell_id)
{
    return SpellDb::battle_rez_ids.find(spell_id) !=
           SpellDb::battle_rez_ids.end();
}
...
bool is_ignorable_event(const std::string& event_type)
{
    return SpellDb::ignorable_events.find(event_type) !=
           SpellDb::ignorable_events.end();
}

bool is_interrupt(int spell_id)
{
    return SpellDb::interrupt_ids.find(spell_id) !=
           SpellDb::interrupt_ids.end();
}
...
bool is_crowd_control(int spell_id)
{
    return SpellDb::crowd_control_ids.find(spell_id) !=
           SpellDb::crowd_control_ids.end();
}
